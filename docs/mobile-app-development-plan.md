...
- S25 Ultra의 알림 접근, 배터리 최적화, 재부팅과 회차별 회귀 검증
- 작은 화면 폭과 큰 글꼴을 조합한 S24 레이아웃 사전 점검
...
**목표:** 가족 기기에 업데이트 가능한 비공개 APK를 배포하고 안정화 판단을 위한
운영 절차를 시작한다.
...
- S25 Ultra 최초 설치, 로그인, 알림 접근 설정, 샘플 거래 등록
- 두 번째 버전 APK 덮어쓰기 테스트로 업데이트·데이터 유지 확인
- 전체 기능 고정 후 S24 스모크와 Fold5 접기·펼치기 호환성 검증
...
- S25 Ultra에 전용 서명 APK가 설치되고 다음 버전으로 덮어쓰기된다.
- S24 스모크와 Fold5 최종 호환성 검증이 통과한다.
...
| 시나리오                | Fold5 최종 호환성 | S24 최종 스모크 | S25 Ultra 회차별 |
| ----------------------- | :---------------: | :-------------: | :--------------: |
| 최초 설치·카카오 로그인 |       필수        |      필수       |       필수       |
| 앱 업데이트·데이터 유지 |       필수        |      필수       |       필수       |
| 알림 접근 허용·회수     |     상태 확인     |      제외       |       필수       |
| 앱 종료 중 알림 수신    |       제외        |      제외       |       필수       |
| 재부팅 후 알림 수신     |       제외        |      제외       |       필수       |
| 오프라인 후보 보존      |     상태 확인     |      제외       |       필수       |
| 같은 알림 중복 방지     |       제외        |      제외       |       필수       |
| 접기·펼치기·다중 창     |       필수        |    해당 없음    |    해당 없음     |
| 큰 글꼴·화면 확대       |       필수        |      제외       |       필수       |
| 배터리 절전 상태        |       제외        |      제외       |       필수       |

S25 Ultra를 개발 기준 기기로 사용한다. S24는 전체 기능과 UI가 고정된 뒤 설치,
업데이트, 로그인과 주요 탭만 확인한다. Fold5는 같은 시점에 커버·내부 화면,
접기·펼치기, 회전·다중 창, 모달·키보드와 작성 중 상태 유지를 검증한다. 에뮬레이터는
레이아웃·권한 회귀 확인용이며 카드사 실제 알림 검증을 대체하지 않는다.
...
| 12회차     | APK 전달 채널                     | USB 또는 Quick Share 직접 전달                |
